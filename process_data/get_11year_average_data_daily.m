    ...
    if strcmp(type_i, 'topunit')
     ...
     for doy_i = 1:365        
        for year_i = 2000:2010
            %% import data
            date_i = datetime(year_i, 1, doy_i);
            month_i = month(date_i);
            day_i = day(date_i);
             
            if strcmp(type_i, 'topunit')
            
            file_top_tmp = [DataTopDir 'elm_plevel_top_11years_' type_i '_grid' num2str(grid_i) '_CLM_USRDAT.ICLM45.intel.clm2.h1.' num2str(year_i)  '-' num2str(month_i, '%02d') '-' num2str(day_i, '%02d')  '-00000.nc'];
            file_pp_tmp = [DataPPDir 'elm_plevel_notop_11years_' type_i '_grid'  num2str(grid_i) '_CLM_USRDAT.ICLM45.intel.clm2.h1.' num2str(year_i)   '-' num2str(month_i, '%02d') '-' num2str(day_i, '%02d')  '-00000.nc'];
            else
            file_top_tmp = [DataTopDir 'elm_top_11years_' type_i '_grid' num2str(grid_i) '_CLM_USRDAT.ICLM45.intel.clm2.h1.' num2str(year_i)   '-' num2str(month_i, '%02d') '-' num2str(day_i, '%02d')  '-00000.nc'];
            file_pp_tmp = [DataPPDir 'elm_notop_11years_' type_i '_grid'  num2str(grid_i) '_CLM_USRDAT.ICLM45.intel.clm2.h1.' num2str(year_i)  '-' num2str(month_i, '%02d') '-' num2str(day_i, '%02d')  '-00000.nc'];
            end
        ...
        %% save
        save([ 'daily/daily_11year_average_' type_i '_grid_' num2str(grid_i) '_plevel_day_'  num2str(day_i) '.mat'], 'lhs_tops',  'shs_tops',   'rhos_tops',  'tsas_tops',    'qs_tops' ,       'ufs_tops',         'psurfs_tops', ...
